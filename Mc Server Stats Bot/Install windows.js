...
if (process.platform !== 'win32') {
    console.log('\n❌ ERROR: This is the Windows installer!\n');
    console.log('   For Linux/Mac, use: node install-unix.js\n');
    process.exit(1);
}
...
console.log('🪟 Windows Installation Mode\n');
...
if (nodeMajor < 16) {
    console.log('\n❌ ERROR: Node.js 16 or higher required!');
...
console.log('📋 Step 2: Checking project files...\n');

if (!fs.existsSync('package.json')) {
    console.log('❌ ERROR: package.json not found!');
    console.log('   Are you in the bot directory?\n');
    rl.close();
    process.exit(1);
}
...
if (!hasNodeModules) {
    ...
    try {
        ...
        console.log('   Try manually: npm install\n');
        rl.close();
        process.exit(1);
    }
} else {
...
console.log('🧹 Step 4: Cleaning up Unix scripts...\n');
...
if (fs.existsSync(debugDir)) {
    ...
    if (removedCount > 0) {
        console.log(`\n   ✅ Removed ${removedCount} Unix script(s)!\n`);
    } else {
        console.log('   ℹ️  No Unix scripts found\n');
    }
}
...
if (fs.existsSync('install-unix.js')) {
    try {
        ...
        console.log('   🗑️  Removed: install-unix.js\n');
    } catch (e) {}
}
...
console.log('⚙️  Step 5: Configuration setup...\n');
...
if (!hasConfig && hasExample) {
    console.log('   📋 Creating global-config.json from example...\n');
...
async function setupToken() {
    ...
    if (hasConfig) {
        ...
        if (!config.token || config.token === 'YOUR_BOT_TOKEN_HERE') {
            ...
            console.log('      5. Paste it here or edit global-config.json later\n');
            
            const answer = await question('   Do you want to enter your token now? (y/n): ');
            ...
            if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
                ...
                if (token && token.length > 50) {
                    ...
                    console.log('\n   ⚠️  Invalid token - please edit global-config.json manually\n');
                }
            } else {
                console.log('\n   ℹ️  You can add your token later in global-config.json\n');
            }
        } else {
...
console.log('📁 Step 7: Creating directories...\n');
...
(async () => {
    ...
    console.log('🧪 Step 8: Running quick test...\n');

    try {
        const testScript = 'Debug\\debug-check.bat';
        ...
        if (fs.existsSync(testScript)) {
            ...
            console.log('   ℹ️  Debug tools not found - skipping tests\n');
        }
    } catch (e) {
    ...
    console.log('   node Debug\\master-debug.js        - Run all tests');
    console.log('   Debug\\Test_all_debug_tools.bat    - Test everything');
    console.log('   Debug\\quick-test.bat              - Quick health check');
    console.log('');

    console.log('📚 Documentation:\n');
    console.log('   README.md              - Full documentation');
    console.log('   Project_Structure.md   - Project overview');
    console.log('   Debug\\Debug-README.md - Debug tools guide');
    console.log('');

    console.log('💡 Need help? https://github.com/Gamer100309/MC-Server-Status-Bot/issues\n');
